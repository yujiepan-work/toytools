...
class Launcher:
    ...
    async def _launch_task(self, task: Task, task_id: int, resource_manager: ResourceManager) -> str:
        async with resource_manager.allocate(quantity=task.cuda_quantity) as cuda_list:
            ...
            logger.info('Running Task-%d CUDA=%s: %s', task_id, cuda, task.cmd_str())
            ...
            logger.warning('%s Task-%d CUDA=%s: %s', status, task_id, cuda, task.cmd_str())
            return status
    ...
    async def _run_single_process(self, cmd: str, io_folder: Union[Path, str], cwd: str, env: dict[str, str]):
        ...
        with open(io_folder / "stdout.log", "w", encoding="utf-8") as f_out, \
            ...
            proc = await asyncio.create_subprocess_shell(
                cmd, stdout=f_out, stderr=f_err, cwd=cwd, env=env
            )
            await proc.wait()
            return proc
